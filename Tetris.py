...
class Tetris(object):
    ...
    def add_shape(self):
        if self.shape_in_play == None:
            self.shape_in_play = Tetris_Shape(random.randint(0,6), random.randint(1,4))
            self.coordinates_shapeip_in_grid = self.map_shape_to_grid(self.place_add_shape, self.shape_in_play) 
    ...
    def map_shape_to_grid(self, place, shape_in_play):
        if self.shape_in_play is not None:
            ...
            for shape_coordinate in coordinates_in_shape:
                coordinate_in_grid = np.array(shape_coordinate) + np.array(place)
    ...
    def check_collision(self):
        ...
        numbers_in_coordinates = [num for coordinate in self.coordinates_shapeip_in_grid for num in coordinate]
    ...
    def flip_shape_ip(self):
        ...
        new_coordinates = self.map_shape_to_grid(self.origin_of_shapeip, shapeip_copy)
    ...
    def move_side(self, direction):        
        column_coordinates = [num for coordinate in self.coordinates_shapeip_in_grid for num in coordinate if num == coordinate[1]]
        ...
        if (direction == 'right' and against_wall != 'right_wall') or (direction == 'left' and against_wall != 'left_wall'):
            ...
            for coordinate in self.coordinates_shapeip_in_grid:
                if direction == 'left':
                    new_coordinates.append(np.array(coordinate) - np.array([0,1]))
                    new_origin_shapeip = np.array(self.origin_of_shapeip) - np.array([0,1])
    ...
    def check_tetris(self):
        ...
        for i in range(self.rows):
            ...
            if len(grid_set) == 1 and grid_set.pop() == '1':
                ...
                self.grid = np.insert(self.grid, 0, np.array([' ']*self.columns), 0)
...
class Tetris_Shape(object):
    ...
    def coordinates_w_1(self):
        coordinates = [(i,j) for i in range(self.shape.shape[0]) for j in range(self.shape.shape[1]) if self.shape[i][j] == 1 or self.shape[i][j] == '1']
        return coordinates
